...
function ContactTypes() {
  ...
  const handleAdd = async () => {
    ...
    try {
      if (editId) {
        await api.put(`/admin/contact-types/${editId}`, { name: newName });
      } else {
        await api.post('/admin/contact-types', { name: newName, order: types.length + 1 });
      }
      setNewName('');
      setEditId(null);
      setShowAddModal(false);
      fetchTypes();
    } catch(err) { console.error('Error saving contact type'); }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteId(null);
      fetchTypes();
    } catch(err) { console.error('Error deleting contact type'); }
  };
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-96">
            <h3 className="text-lg font-bold mb-4 text-gray-800">{editId ? 'Edit Contact Type' : 'Add New Contact Type'}</h3>
            <input 
              type="text"  
